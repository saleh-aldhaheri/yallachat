<div>
    @php
        $channel = \App\Enums\NotificationChannels::WHATSAPP;
    @endphp
    ...
    @if($showModal)
        <x-modal max-width="max-w-md" :close="'cancel'">
            <div class="flex justify-between items-center mb-6">
                <h5 class="text-them font-mono text-base">{{ $reconnect ? 'Reconnect WhatsApp' : 'Connect WhatsApp' }}</h5>
            ...
            <form wire:submit.prevent="submit">
                <p class="text-ink-muted text-xs font-mono mb-5">
                    @if($codeSent)
                        We sent a verification code to <span class="text-ink">{{ $phoneNumber }}</span>.
                        Enter it below to confirm.
                    @elseif($reconnect)
                        Reconnect notifications for <span class="text-ink">{{ $phoneNumber }}</span>.
                ...
                <x-form-field label="phone_number" name="phoneNumber">
                    <x-form-input type="tel" wire:model="phoneNumber" placeholder="+1234567890"
